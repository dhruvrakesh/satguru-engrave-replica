-- Phase 1: Data population for Satguru (checking correct column names first)

-- Add sample flexible packaging categories for Satguru (using correct column structure)
INSERT INTO public.satguru_categories (category_name, description, is_active) VALUES
...
DECLARE
    ...
    barrier_cat_id uuid;
BEGIN
    -- Get category IDs using correct column name
    SELECT id INTO cat_id FROM public.satguru_categories WHERE category_name = 'Flexible Packaging Films' LIMIT 1;
    SELECT id INTO film_cat_id FROM public.satguru_categories WHERE category_name = 'Printed Films' LIMIT 1;
    SELECT id INTO lam_cat_id FROM public.satguru_categories WHERE category_name = 'Laminated Films' LIMIT 1;
    SELECT id INTO barrier_cat_id FROM public.satguru_categories WHERE category_name = 'Barrier Films' LIMIT 1;
