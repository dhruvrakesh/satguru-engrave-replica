...
-- Create initial stock entries for Satguru items (using correct schema)
INSERT INTO public.satguru_stock (item_code, opening_qty, current_qty, last_updated)
SELECT 
  sim.item_code,
...
-- Create sample GRN entries for Satguru (using correct schema)
INSERT INTO public.satguru_grn_log (grn_number, date, item_code, uom, qty_received, invoice_number, amount_inr, vendor, remarks)
SELECT 
...
-- Create sample Issue entries for Satguru (using correct schema)
INSERT INTO public.satguru_issue_log (date, item_code, qty_issued, purpose, total_issued_qty, remarks)
SELECT 
...
-- Create daily stock summary entries for Satguru (using correct schema)
INSERT INTO public.satguru_daily_stock_summary (item_code, item_name, category_name, summary_date, opening_qty, total_grn_qty, total_issued_qty, current_qty, days_of_cover)
SELECT 
  ss.item_code,
  sim.item_name,
  sc.category_name,
  ...
  ss.current_qty,
  CASE WHEN COALESCE(recent_issue.total_issued, 0) > 0 
       THEN ROUND(ss.current_qty / COALESCE(recent_issue.total_issued, 1), 1)
       ELSE NULL 
  END
FROM public.satguru_stock ss
JOIN public.satguru_item_master sim ON ss.item_code = sim.item_code
LEFT JOIN public.satguru_categories sc ON sim.category_id = sc.id
LEFT JOIN (
